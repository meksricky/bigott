...
class ClientOrderHistory(models.Model):
    ...
    # Use Monetary so Odoo formats amounts correctly
    ...
    # Computed fields
    budget_per_product = fields.Monetary(
        'Budget per Product', currency_field='currency_id',
        compute='_compute_budget_metrics', store=True
    )
    ...
    # ------- Analysis API used by the engine -------
    @api.model
    def analyze_client_patterns(self, partner_id):
        ...
        latest_categories = histories[0].get_category_structure()
    ...
    _EXPERIENCE_THRESHOLD = 0.7   # ≥70% of order amount looks like an experience
    _LOOKBACK_YEARS = 3
    ...
    def rebuild_from_sales(self, date_from=False, date_to=False):
        ...
        for so in orders:
            ...
            buckets[key]['amount'] += order_total_company
    ...
    # Fallback used if no rows exist yet for a partner
    @api.model
    def _analyze_from_sales_fallback(self, partner_id):
        ...
        for so in orders:
            ...
            amount_company = so.currency_id._convert(
                so.amount_total, company_currency, so.company_id,
                so.date_order or so.confirmation_date or fields.Datetime.now()
            )
    ...
    def action_generate_next_composition(self):
        return {
            ...
            'context': {
                ...
                'default_target_year': fields.Date.today().year + 1,
                'default_target_budget': self.total_budget * 1.10 if self.total_budget else 0.0,
            }
        }
