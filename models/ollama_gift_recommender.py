...
class OllamaGiftRecommender(models.Model):
    ...
    def _parse_notes_inline(self, notes):
        ...
        parsed = {
            ...
            'categories_required': {},
            'categories_excluded': [],
            'budget_flexibility': 0.05,  # Default 5%
            'preferences': {}
        }
        ...
        if 'flexible budget' in notes_lower:
            parsed['budget_flexibility'] = 0.20  # 20% flexibility
        elif 'strict budget' in notes_lower:
            parsed['budget_flexibility'] = 0.05  # 5% flexibility
    ...
    def generate_gift_recommendations(self, partner_id, target_budget, 
        ...
        if notes_requirements.get('mandatory_count'):
            return self._generate_with_mandatory_count(
    ...
    def _generate_with_mandatory_count(self, partner_id, target_budget, 
        ...
        if not products:
            raise UserError("No products available matching criteria")
        ...
        for category, count in notes_requirements.get('categories_required', {}).items():
            cat_products = [
                p for p in products 
                if category.lower() in (getattr(p, 'lebiggot_category', '') or '').lower()
            ]
        ...
        if remaining_count > 0:
            ...
            if notes_requirements.get('preferences', {}).get('loves_sweets'):
                sweet_products = [
                    ...
                    if 'dulce' in (getattr(p, 'lebiggot_category', '') or '').lower()
                    or 'sweet' in p.name.lower()
                ]
    ...
    def _build_ollama_prompt_with_notes(self, partner, target_budget, client_notes, 
        ...
        product_list = ""
        for i, p in enumerate(products[:50], 1):  # Limit to 50 products
            product_list += f"{i}. ID:{p.id} | {p.name} | €{p.list_price:.2f} | {getattr(p, 'lebiggot_category', 'General')}\n"
...
{{
    ...
    def _select_products_intelligently(self, products, target_budget):
        ...
        # CORRECT: Target 95-105% of budget
        min_target = target_budget * 0.95
        max_target = target_budget * 1.05
    ...
    def _get_available_products(self, target_budget, dietary_restrictions):
        ...
        min_price = max(10, target_budget * 0.02)
        max_price = target_budget * 0.40  # No single item over 40% of budget
    ...
    def _analyze_client_history(self, partner_id):
        ...
        analysis = {
            'has_history': True,
            'avg_products': sum(h.total_products for h in history) / len(history),
        ...
        for h in history:
            if h.product_ids:
                analysis['recent_products'].extend(h.product_ids.ids)
